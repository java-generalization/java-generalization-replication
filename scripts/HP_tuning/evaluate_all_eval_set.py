...
def test(args, model, tokenizer, test_dataset, checkpoint, best_threshold=0.5):
    # build dataloader
    ...
    df.to_csv(args.output_csv.replace("CHECKPOINT", checkpoint))

    with open(args.output_csv.replace("CHECKPOINT", checkpoint).replace(".csv", "_perfect.txt"), 'w+') as file:
...
def main():
    ...
    checkpoints=["checkpoint-{}".format(c.strip()) for c in checkpoints]

    # checkponts_epoch=[c for c in os.listdir(args.checkpoint_folder) if "epoch" in c]

    # checkpoints.extend(checkponts_epoch)
    ...
    if args.do_test:
        ...
        test_data=read_dataset(args.dataset_folder, "eval")
        ...
        for checkpoint in checkpoints:
            ...
            output_dir = os.path.join(args.checkpoint_folder, checkpoint, "pytorch_model.bin")
            ...
            test(args, model, tokenizer, test_dataset, checkpoint, best_threshold=0.5)
